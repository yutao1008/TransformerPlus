...
def main():
    ...
    if all((opt.train_path, opt.val_path)): # WMT16 dataset
        training_data, validation_data = prepare_dataloaders_from_bpe_files(opt, device)
    elif opt.data_pkl: # WMT17 dataset
        training_data, validation_data = prepare_dataloaders(opt, device)
    else: # IWSLT2017
        from torchtext.datasets import IWSLT2017
        training_data, validation_data, _ = IWSLT2017()
    ...
    optimizer = ScheduledOptim(
        optim.AdamW(transformer.parameters(), betas=(0.9, 0.98), eps=1e-09),
...
def profile():
    ...
    input1 = torch.randint(0,100,(1, 64)).cuda()
    input2 = torch.randint(0,100,(1, 64)).cuda()
    macs, params = profile(transformer, inputs=(input1,input2))
    print('MACS')
